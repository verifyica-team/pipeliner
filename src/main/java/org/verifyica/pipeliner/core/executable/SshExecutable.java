...
/** Class to implement SshExecutable */
public class SshExecutable extends AbstractExecutable {

    private static final Logger LOGGER = LoggerFactory.getLogger(SshExecutable.class);
     ...
     */
    public SshExecutable(Step step, String commandLine) {
    ...
    public int execute(Context context) throws Throwable {
        ...
        try {
            ...
            String[] tokens = resolvedCommandLine.split(" ", 3);

            // Escape the command to execute it on the remote server
            String remoteCommend = tokens[2].replace("\\", "\\\\").replace("\"", "\\\"");

            // Get the shell command
            String shellCommand = shell.name().toLowerCase();

            // Build the process command tokens
            String[] processCommands = new String[] {"ssh", tokens[1], shellCommand, "-c", "\"" + remoteCommend + "\""};
