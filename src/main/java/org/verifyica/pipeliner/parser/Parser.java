...
public class Parser {
    ...
    public static List<ParsedToken> parse(String input) throws SyntaxException {
         ...
         * Phase 1: Merge...
         *
         * BACKSLASH + VARIABLE into a TEXT token
        ...
        for (LexerToken lexerToken : phase2LexerTokens) {
            ...
            switch (type) {
                case VARIABLE: {
                    ...
                    // Check if the variable value is invalid
                    if (isInvalidVariable(value)) {
                        throw new SyntaxException(format(
                                "invalid variable [%s] at position [%d]",
                                lexerToken.getText(), lexerToken.getPosition()));
                    }

                    // Add the VARIABLE token
                    parsedTokens.add(new ParsedToken(ParsedToken.Type.VARIABLE, position, text, value));
    ...
    /**
     * Method to check if a variable value is valid
     *
     * @param value the value
     * @return true if the value is valid, else false
     */
    private static boolean isValidVariable(String value) {
        ...
        if (isNotInString(characters[0], VARIABLE_BEGIN_CHARACTERS)) {
            return false;
        }

        // Validate the remaining characters
        for (int i = 1; i < characters.length; i++) {
            if (isNotInString(characters[i], VARIABLE_REMAINING_CHARACTERS)) {
                return false;
            }
        }
    ...
    /**
     * Method to check if a variable value is invalid
     *
     * @param value the value
     * @return true if the value is invalid, else false
     */
    private static boolean isInvalidVariable(String value) {
     ...
     */
    private static boolean isNotInString(char c, String string) {
